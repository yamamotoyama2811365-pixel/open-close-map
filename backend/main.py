...
app=FastAPI(title="Open Close Map API",version="0.6.0")
...
def tenant_info(store_id:int):
    ...
    return {
        "items":[
            {
                "id":r[0],"source_name":r[1],"source_url":r[2],
                "status":r[3],
...
def activity_score(store_id:int):
    ...
    if score>=80:
        label="非常に活発";comment="店舗の出入りが多く、商業動向が活発なエリアです。"
    elif score>=65:
        label="活発";comment="新規出店や店舗入替が比較的多いエリアです。"
    elif score>=50:
        label="標準";comment="一定の店舗動向が確認できるエリアです。"
    else:
        label="データ蓄積中";comment="現時点では店舗情報が少なく、今後のデータ蓄積で評価が変わる可能性があります。"
...
def enrich_addresses(limit:int=Query(default=30,ge=1,le=100)):
    if not DATABASE_URL:
        return {"ok":False,"error":"DATABASE_URL is not configured"}
    return {"ok":True,**enrich_candidates(DATABASE_URL,limit=limit,only_unprocessed=True)}
