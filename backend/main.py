...
app=FastAPI(title="Open Close Map API",version="1.2.1")
...
@app.get("/")
def root():return {"service":"open-close-map-api","status":"ok","version":"1.2.1","time":datetime.now(timezone.utc).isoformat()}
@app.get("/health")
...
def rescue_source_batch():
    """
    Google News URLを解読せず、記事タイトルと配信元から元記事を探し直す。
    1回5件固定。
    現在はPR TIMESを優先対応。
    """
    if not DATABASE_URL:
