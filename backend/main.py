    ...
    with conn:
        with conn.cursor() as cur:
            cur.execute("""
                ...
                RETURNING id
            """, (
                store.name,store.status,store.category,store.prefecture,
                store.city,store.address,store.open_date,store.close_date,
...
def discovery(
        ...
        with conn.cursor() as cur:
            cur.execute(f"""
                SELECT id,title,source_name,source_url,published_at,
                       detected_status,prefecture,city,confidence,
                       processed,store_name_candidate,event_date_candidate,
                       category_candidate,created_at
                FROM discovery_items
                {where}
    ...
    items = [{
        ...
        "detected_status":r[5],"prefecture":r[6],"city":r[7],
        "confidence":r[8],"processed":r[9],"store_name_candidate":r[10],
        "event_date_candidate":r[11].isoformat() if r[11] else None,
